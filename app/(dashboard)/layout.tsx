      ...
      <aside className="w-56 border-r border-zinc-800/80 bg-[#0d0e12] p-4 flex flex-col justify-between shrink-0 h-full">
        <div>
          ...
          <div className="flex items-center gap-2.5 mb-6 px-1">
            ...
            <div>
              ...
              <span className="text-[10px] text-zinc-400 block">Work Dashboard</span>
            </div>
          </div>

          {/* Danh mục menu điều hướng */}
          <div>
            <span className="text-[9px] text-zinc-400 font-bold uppercase tracking-wider block mb-2 px-2">Workspace</span>
              ...
              {menuItems.map((item, index) => {
                ...
                return (
                  <Link
                    key={index}
                    href={item.href}
                    className={`w-full flex items-center gap-2.5 px-2.5 py-1.5 rounded-md text-[12px] font-medium transition-all ${isActive
        ...
        <div className="flex items-center gap-2.5 pt-3.5 border-t border-zinc-800/80 px-1">
          ...
          <div className="min-w-0">
            <p className="text-[9px] text-zinc-400 truncate">© 2026 SecurityZone Team</p>
          </div>
        </div>
      </aside>
